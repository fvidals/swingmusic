...
from services.hash_utils import compute_runtime_trackhash, create_hash
        ...
        with swing_db() as conn:
            for item in parsed_items:
                ...
                if row:
                    ...
                    try:
                        artists = json.loads(artists_raw) if artists_raw and artists_raw.startswith("[") else ([artists_raw] if artists_raw else [])
                        artist_names = ", ".join(a.get("name", "") if isinstance(a, dict) else str(a) for a in artists)
                    except Exception:
                        artist_names = str(artists_raw or "")

                    # Compute exact SwingMusic in-memory runtime trackhash
                    thash = compute_runtime_trackhash(title, album, artists_raw) if title else track_data.get("trackhash")
